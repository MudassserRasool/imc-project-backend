...
var _workoutModel = _interopRequireDefault(
  require('../models/workoutModel.js')
);
...
const getWorkouts = async (req, res) => {
  try {
    const user_id = req.user._id;
    const workouts = await _workoutModel.default
      .find({
        user_id,
      })
      .sort({
...
const getWorkout = async (req, res) => {
  ...
  try {
    const workout = await _workoutModel.default.findById(id);
    res.status(200).json(workout);
  } catch (error) {
    res.status(404).json({
      message: error.message,
...
const createWorkout = async (req, res) => {
  ...
  if (emptyError.length > 0) {
    return res.status(400).json({
      error: `please add ${emptyError}`,
    });
  }
  try {
    const user_id = req.user._id;
    const workout = await _workoutModel.default.create({
      title,
      load,
      reps,
      user_id,
...
const updateWorkout = async (req, res) => {
  ...
  await _workoutModel.default.findByIdAndUpdate(id, updatedWorkout, {
    new: true,
  });
  res.json(updatedWorkout);
};
...
const deleteWorkout = async (req, res) => {
  ...
  const workout = await _workoutModel.default.findByIdAndDelete(id);
  if (!workout) {
    return res.status(400).json({
  ...
  res.status(200).json({
    message: `workout of id ${id} is delete scuessfully`,
  });
};
